...
GPU implementation. The OPF 375 matrix is mostly diagonal, thus the
vector $\Vc{x}$ is read in close quarters, randomization breaks it.
If the load balance is fixed (i.e., by dividing the matrix by row and
in equal row), randomization is beneficial.
...
\doublefigure{.45}{../python/ELLEPNG/lp_osa_07_mtx_GPU_64_COO.png}{../python/ELLEPNG/lp_osa_07_mtx_GPU_64_CSR.png}{Ellesmere, GPU 64bits COO (left) and GPU CSR (right) for OPF 3754}{fig:eight1}
\doublefigure{.45}{../python/FIJIPNG/lp_osa_07_mtx_GPU_64_COO.png}{../python/FIJIPNG/lp_osa_07_mtx_GPU_64_CSR.png}{Fiji, GPU 64bits COO (left) and GPU CSR (right) for OPF 3754}{fig:eight1}
...
\ref{fig:10}, and \ref{fig:11} and the gains can be up to
...
performance with other GPUs or even between CPUs and GPUs. Often the
limitation is the software.
...
may affect precision but please remember that these are sparse
matrices and the number of multiplication and accumulations are
practically a small constant.  For the semiring (min,+) and for
integer arithmetic the computation order does not matter because there
is no intermediary error.
